...
def cam_master():
    ...
    while not rospy.is_shutdown():
        ret1, front_cam_left_frame = front_cam_left_cap.read()
        ret2, front_cam_right_frame = front_cam_right_cap.read()
        ret3, bottom_cam_frame = bottom_cam_cap.read()

        # image scaling or modification should happen here if necessary

        # converting the frames to ros images
        front_cam_left_frame_msg = bridge.cv2_to_imgmsg(front_cam_left_frame, "bgr8")
        front_cam_right_frame_msg = bridge.cv2_to_imgmsg(front_cam_right_frame, "bgr8")
        bottom_cam_frame_msg = bridge.cv2_to_imgmsg(bottom_cam_frame, "bgr8")
